...
const char DEFAULT_WALLET_FILE_NAME[] = "una.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultNodeList << "35.231.85.99:44415";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("unawallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("unaallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("unawallet.desktop");
  ...
  res = autorunSettings.contains("unawallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("unawallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    QString autorunFilePath = autorunDir.absoluteFilePath("unawallet.plist");
    QSettings autorunSettings(autorunFilePath, QSettings::NativeFormat);
    autorunSettings.remove("Program");
    autorunSettings.setValue("Label", "africa.una.unawallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=UNA Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("unawallet", appPath);
    } else {
      autorunSettings.remove("unawallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  ...
  protocolSettings.setValue(".", "URL:una");
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("unawallet.cfg"));
